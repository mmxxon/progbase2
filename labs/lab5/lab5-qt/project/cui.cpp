...
void Cui::OrgsMainMenu() {
  ...
  int l1 = 3;
  for (auto &i : orgs) { if ((int)to_string(i.id).length() > l1) { l1 = to_string(i.id).length() + 3; } }
  cout << yel_f << left << setw(l1) << "id" << yel_f "label" res << endl;
  for (auto& org: orgs) {
    cout << cya_f << left << setw(l1) << org.id << mag_f << org.label << res << endl;
  }
  cout << cont << endl;
  getch();
  system("clear");
...
void Cui::OrgsMenu(int org_id) {
  ...
  int l1(2), l2(10), l3(8);
  if ((int)to_string(org.value().id).length() >= l1) l1 = to_string(org.value().id).length() + 3;
  if ((int)org.value().country.length() >= l2) l2 = org.value().country.length() + 3;
  if ((int)org.value().label.length() >= l3) l3 = org.value().label.length() + 3;
  cout << cya_f "Found organisation:" res << endl;
  cout << yel_f << left << setw(l1) << "id"
       << yel_f << left << setw(l2) << "country"
       << yel_f << left << setw(l3) << "label"
       << yel_f << left << "founders" res << endl;
  cout << blu_f << left << setw(l1) << org.value().id
       << mag_f << left << setw(l2) << org.value().country
       << blu_f << left << setw(l3) << org.value().label
       << mag_f << left << org.value().founders << res << endl;
  cout << cont << endl;
  getch();
  system("clear");
}
void Cui::OrgsUpdateMenu(int org_id) {
  ...
  while(1) {
    ...
    cout << yel_f << left << setw(10) << "Id: " cya_f + to_string(o.id)
         << yel_f << right << setw(45) << "Label: " mag_f + o.label
         << res << endl
         << yel_f << left << setw(10) << "Country: " cya_f + o.country
         << yel_f << right << setw(45) << "Founders: " mag_f + o.founders
         << res << endl;
...
void Cui::OrgsDeleteMenu(int org_id) {
  ...
  if (!storage_->removeOrg(org_id)) {
    ...
    cout << yel_f "Deleted" mag_f " > " red_f << org.value().label << res << endl;
  }
  cout << cont << endl;
  getch();
  system("clear");
...
void Cui::FndrsMainMenu() {
  ...
  int l1 = 3;
  for (auto &i : fndrs) { if ((int)to_string(i.id).length() > l1) { l1 = to_string(i.id).length() + 3; } }
  cout << yel_f << left << setw(l1) << "id" << yel_f "name" res << endl;
  for (auto& fndr: fndrs) {
    cout << cya_f << left << setw(l1) << fndr.id << mag_f << fndr.name << res << endl;
  }
  cout << cont << endl;
  getch();
  system("clear");
}
void Cui::FndrMenu(int fndr_id) {
  system("clear");
  ...
  int l1(2), l2(10), l3(8);
  if ((int)to_string(fndr.value().id).length() >= l1) l1 = to_string(fndr.value().id).length() + 3;
  if ((int)fndr.value().name.length() >= l2) l2 = fndr.value().name.length() + 3;
  if ((int)to_string(fndr.value().age).length() >= l3) l3 = to_string(fndr.value().age).length() + 3;
  cout << cya_f "Found organisation:" res << endl;
  cout << yel_f << left << setw(l1) << "id"
       << yel_f << left << setw(l2) << "name"
       << yel_f << left << setw(l3) << "age"
       << yel_f << left << "wealth" << res << endl;
  cout << blu_f << left << setw(l1) << fndr.value().id
       << mag_f << left << setw(l2) << fndr.value().name
       << blu_f << left << setw(l3) << fndr.value().age
       << mag_f << left << fndr.value().wealth << res << endl;
  cout << cont << endl;
  getch();
  system("clear");
}
void Cui::FndrUpdateMenu(int fndr_id) {
  int key;
  optional<Founder> fndr = storage_->getFndrById(fndr_id);
  if (!fndr) {
    system("clear");
  ...
  while(1) {
    ...
    cout << yel_f << left << setw(10) << "Id: " cya_f + to_string(f.id)
         << yel_f << right << setw(45) << "Name: " mag_f + f.name
         << res << endl
         << yel_f << left << setw(10) << "Age: " cya_f + to_string(f.age)
         << yel_f << right << setw(45) << "Wealth: " mag_f + f.wealth
         << res << endl;
    cout << yel_f "Actions" << endl <<endl;
    ...
    if (key ==52) {
      ...
      string tmp;
      getline(cin, tmp);
      if (isdigit(tmp[0])) f.age = stoi(tmp);
      else {
...
void Cui::FndrDeleteMenu(int fndr_id) {
  optional<Founder> fndr = storage_->getFndrById(fndr_id);
  system("clear");
  if (!storage_->removeFndr(fndr_id)) {
    cout << red_f "Wrong id" res << endl;
  } else {
    cout << yel_f "Deleted" mag_f " > " red_f << fndr.value().name << res << endl;
  }
  cout << cont << endl;
  getch();
  system("clear");
...
void Cui::FndrCreateMenu() {
  ...
  string tmp;
  getline(cin, tmp);
  if (isdigit(tmp[0])) fndr.age = stoi(tmp);
  else {
    cout << red_f "Wrong age" res << endl;
    cout << cont << endl;
    getch();
        ...
        if (key == 52) { system("clear"); break; }
          ...
          if (isdigit(tmp[0])) {
            ...
            while(1) {
              ...
              if ( key == 49 ) OrgsMenu(id);
              else if ( key == 50 ) OrgsUpdateMenu(id);
              else if ( key == 51 ) { OrgsDeleteMenu(id); break; }
              else if ( key == 52 ) { system("clear"); break; }
            }
          } else {
        ...
        if (key == 52) { system("clear"); break; }
          ...
          if (isdigit(tmp[0])) {
            ...
            while(1) {
              ...
              if ( key == 49 ) FndrMenu(id);
              else if ( key == 50 ) FndrUpdateMenu(id);
              else if ( key == 51 ) { FndrDeleteMenu(id); break; }
              else if ( key == 52 ) { system("clear"); break; }
            }
          } else {
            cout << red_f "Wrong input" res << endl;

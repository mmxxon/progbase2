...
void EditDialog::on_okay_button_clicked() {
  ...
  org->label = (ui->edit_label->text().length() != 0)?ui->edit_label->text().toStdString():ui->edit_label->placeholderText().toStdString();
  org->country = (ui->edit_country->text().length() != 0)?ui->edit_country->text().toStdString():ui->edit_country->placeholderText().toStdString();
  org->founders = (ui->edit_founders->text().length() != 0)?ui->edit_founders->text().toStdString():ui->edit_founders->placeholderText().toStdString();

  emit upd_org(org);
  EditDialog::close();
}

void EditDialog::on_cancer_button_clicked() {
  EditDialog::close();
}
